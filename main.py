...
# ===== لیست‌های واژگان (همان قبلی، بدون تغییر) =====
PERSONS = {
...
PLACES = { ... }  # (کامل مطابق کد قبلی، از پیش پر شده)

MILITARY = { ... }  # (کامل مطابق کد قبلی)
...
# ===== لیست سیاه قدرتمند (ترکیبی) =====
BLOCKED_WORDS = [
    # کلمات عمومی تبلیغ و کلاهبرداری
    ...
    # کمک‌های مالی و خیریه (همه واژه‌های مرتبط)
    "شماره کارت", "شبا", "واریز", "کمک مالی", "خیریه", "خیریه نیک",
    "پرونده بیمار", "مجوزها", "تسویه", "مازاد کمک", "تلگرام نیک",
    "سایت خیریه", "PoshtibaniDarman", "کودکان محروم",
    "داروهای گران", "چشم انتظار همت", "در توان مادر",
    "پرونده‌های درمان", "گزارش پرونده", "مازاد کمک‌ها",
    "صرف امورات مؤسسه", "مؤسسه خیریه", "هزینه درمان",
    # عبارات مخصوص فوتر تبلیغاتی «آخرین خبر»
...
async def handler(event):
    try:
        ...
        # سدهای دفاعی چندلایه
        if contains_blocked(text) or has_financial_scam(text) or too_many_links(text):
            print("⛔ تبلیغ یا کلاهبرداری")
            return
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات ضدگلوله احسان روشن شد.")
        with client:
