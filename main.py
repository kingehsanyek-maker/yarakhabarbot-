...
def is_spam_line(line, prev_line=None, next_line=None):
    """تشخیص خط تبلیغاتی با کمک خطوط همسایه"""
    ...
    # این کار از آسیب به جملات خبری جلوگیری می‌کند.
    if re.search(r'اخبار', line) and re.search(r'(ببینید|دنبال کنید)', line):
        # اگر خط قبلی یا خط بعدی «وب‌سایت:» یا «تلگرام:» نباشد، این جمله می‌تواند بخشی از خبر باشد
        ...
        if next_line and re.search(r'(وب‌سایت|تلگرام|اینستاگرام)\s*:', next_line):
            return True
        # اگر خط قبلی یا بعدی شامل لینک تبلیغاتی باشد، احتمالاً فوتر است
        if prev_line and re.search(r'https?://', prev_line):
            return True
        if next_line and re.search(r'https?://', next_line):
            return True
        # در غیر این صورت، این جمله بخشی از متن خبری است – حذف نمی‌شود
        return False
    return False
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات هوشمند (ضد تبلیغات ساختاری) روشن شد.")
        with client:
