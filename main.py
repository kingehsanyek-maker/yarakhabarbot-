...
PLACES = { ... }  # (همان لیست کامل قبلی، اینجا برای اختصار حذف شد - لطفاً عیناً از آخرین کد کامل کپی کن)
MILITARY = { ... }
SPORTS_CONTEXT = [ ... ]
TOPICS = { ... }
EXTRA_TOPICS = { ... }
...
# ===== لیست سیاه کلمات (فقط برای بلاک کامل) =====
BLOCKED_WORDS = [
    ...
    # کلاهبرداری مالی (با اینها کل خبر بلاک می‌شود)
    "شماره کارت", "شبا", "واریز", "کمک مالی",
...
# امضاهایی که همیشه از متن حذف می‌شوند (نه بلاک)
TEXTS_TO_REMOVE = [
    ...
    "https://rubika.ir/Akharinkhabar",
    "https://eitaa.com/akharinkhabar",
    "https://ble.ir/akharinkhabar",
    "https://splus.ir/akharinkhabar",
    "https://gap.im/akharinkhabar",
    "https://play.google.com/store/apps/details?id=com.khorasannews.akharinkhabar",
    "http://akhr.ir/AK7",
]

# عبارات تبلیغاتی که کل خطشان پاک می‌شود
PROMO_LINE_PHRASES = [
...
def remove_promotional_lines(text):
    """خط‌هایی که لینک پیام‌رسان‌های داخلی یا خود آخری‌ن‌خبر دارند، کاملاً حذف می‌کند."""
    lines = text.split('\n')
    new_lines = []
    for line in lines:
        # لینک‌های روبیکا، بله، ایتا، سروش، گپ
...
def clean_text(text):
    if not text: return ""
    # اول خط‌های تبلیغاتی را حذف کن
    text = remove_promotional_lines(text)
    # سپس کلمات و عبارات مشخص را پاک کن
    for item in TEXTS_TO_REMOVE:
        text = text.replace(item, "")
    # باقی‌ماندهٔ منشن‌ها (هرچیزی که با @ شروع می‌شود)
    text = re.sub(r"@[^\s]+", "", text)
    # لوله‌ها و فاصله‌های اضافی
    text = re.sub(r"\|\s*", "", text)
    # خط‌های خالی متوالی را به یک خط تبدیل کن
    text = re.sub(r"\n\s*\n+", "\n", text)
    return text.strip()
...
async def handler(event):
    try:
        ...
        # فقط کلاهبرداری‌های مالی و تبلیغات واضح را بلاک کن
        if contains_blocked(text) or has_financial_scam(text):
            print("⛔ تبلیغ / کلاهبرداری")
            return

        # تمیزکاری هوشمند (لینک‌های پیام‌رسان‌ها و امضاها پاک می‌شوند)
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات هوشمند احسان (پاک‌کننده لینک) روشن شد.")
        with client:
