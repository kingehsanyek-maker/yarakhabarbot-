...
from telethon import TelegramClient, eventsimport os, time, asyncio, re, difflib, threading
...
TEXTS_TO_REMOVE = [
    ...
    "Www.omidbank.ir", "www.omidbank.ir",
]
...
def is_spam_line(line):
    ...
    if re.search(r'https?://', line) and re.search(r'(کلیک کنید|همین حالا|بازدید کنید|ثبت نام|همین الان|برای اطلاعات بیشتر|جهت مشاهده|اطلاعات بیشتر|اینجا کلیک کنید|برای خرید|تخفیف ویژه|همین الان عضو شوید|همین الان دانلود کنید)', line, re.IGNORECASE):
        return True
    if re.search(r'(کارت\s?هدیه|غیر\s?حضوری|تحویل بگیر|با امید بانک|طرح ویژه|اعتبار|سپرده|سود علی‌الحساب)', line):
        return True
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات خودکفای احسان (همه چیز آماده) روشن شد.")
        with client:
...
TEXTS_TO_REMOVE = [
    ...
    "@Projectmeshkat", "t.me/Projectmeshkat",
    "https://zil.ink/ProjectMeshkat",
    # دامنه‌های بانکی و تبلیغاتی
    "Www.omidbank.ir", "www.omidbank.ir", "omidbank.ir",
]
...
def is_spam_line(line):
    ...
    if re.search(r'https?://t\.me/akharinkhabar', line, re.IGNORECASE):
        return True
    # هر خطی که akharinkhabar.ir داشته باشد
    if re.search(r'akharinkhabar\.ir', line, re.IGNORECASE):
        return True
    # ترکیب لینک + کلمات ترغیبی
    if re.search(r'https?://', line) and re.search(r'(کلیک کنید|همین حالا|بازدید کنید|ثبت نام|همین الان|برای اطلاعات بیشتر|جهت مشاهده|اطلاعات بیشتر|اینجا کلیک کنید|برای خرید|تخفیف ویژه|همین الان عضو شوید|همین الان دانلود کنید)', line, re.IGNORECASE):
        return True
    # الگوهای تبلیغات بانکی (با فاصله یا بدون فاصله)
    if re.search(r'(کارت\s?هدیه|غیر\s?حضوری|تحویل بگیر|با امید بانک|طرح ویژه|اعتبار|سپرده|سود علی‌الحساب)', line):
        return True
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        threading.Thread(target=memory_cleaner, daemon=True).start()
        threading.Thread(target=run_web, daemon=True).start()
        print("🚀 ربات مینیمال و هوشمند احسان (ضد تبلیغات بانکی) روشن شد.")
        with client:
            client.loop.run_until_complete(resolve_dest())
            client.run_until_disconnected()
