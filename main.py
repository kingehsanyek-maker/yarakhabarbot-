...
# ===== لیست سیاه کلمات واقعاً خطرناک (کلاهبرداری، شرط‌بندی، خیریه) =====
BLOCKED_WORDS = [
...
# نام شبکه‌های اجتماعی ایرانی که ممکن است تبلیغاتی باشند (برای تمیزکاری خط به خط)
IRANIAN_SOCIALS = ["روبیکا", "بله", "ایتا", "سروش", "گپ"]
...
def clean_text(text):
    ...
    # تمیزکاری خط‌به‌خط هوشمند
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        stripped = line.strip()
        # اگر خط فقط ایموجی یا خالی بود، نگه دار
        if not stripped:
            cleaned_lines.append(line)
            continue

        # بررسی کن آیا این خط یک خط تبلیغاتی شبکه‌های اجتماعی ایرانی است
        is_promo_line = False

        # چک کن آیا نام شبکه ایرانی در خط هست
        for social in IRANIAN_SOCIALS:
            if social in stripped:
                # اگر در همان خط لینکی (که پاک شده ولی ممکن است بقایای آن باشد) یا کلمه "👇" یا "⏩" بود، خط حذف شود
                # چون لینک‌ها قبلاً پاک شده‌اند، نشانه‌ها فلش‌ها هستند
                if any(arrow in stripped for arrow in ["👇", "⏩", "🔸", "🔹", "▶"]):
                    is_promo_line = True
                    break
                # اگر خط بعدی (در lines) لینک داشت (قبل از پاک شدن)، اما الان لینک‌ها پاک شده‌اند،
                # بهتر است کلاً خطوطی که شامل این کلمات باشند و خط قبلی یا بعدی لینکی داشته باشند، حذف شوند.
                # اما اینجا ساده‌تر: اگر نام شبکه با "روبیکا" "بله" "ایتا" "سروش" "گپ" باشد و خط حاوی ایموجی فلش باشد.
                # همچنین اگر خط دقیقاً خود نام باشد (مثلاً "روبیکا👇") با ایموجی.
        # عبارت‌های دعوت‌کننده
        invite_phrases = ["مارا دنبال کنید", "به ما بپیوندید", "عضو کانال ما", "کانال ما را دنبال کنید",
                          "همراه با لینک", "ما را دنبال کنید", "دنبال کنید", "عضو شوید"]
        ...
        if is_promo_line:
            continue  # این خط کلاً حذف می‌شود
...
def count_akharinkhabar(text):
    pattern = r'(?i)@akharinkhabar|akharinkhabar\.ir|rubika\.ir/akharinkhabar|eitaa\.com/akharinkhabar|ble\.ir/akharinkhabar|splus\.ir/akharinkhabar|gap\.im/akharinkhabar|instagram\.com/akharinkhabar|t\.me/akharinkhabar|آخرین\s?خبر|اخرین\s?خبر|akhr\.ir'
...
def too_many_links(text, max_links=4):
    links = re.findall(r'https?://\S+', text)
    return len(links) > max_links
...
async def handler(event):
    try:
        ...
        # فقط کلاهبرداری و شرط‌بندی بلاک شوند
        if contains_blocked(text) or has_financial_scam(text):
            print("⛔ کلاهبرداری / شرط‌بندی")
            return

        # اگر بیش از ۲ اشاره به برند آخرین خبر باشد (فوتر تبلیغاتی)، کل خبر بلاک
        if count_akharinkhabar(text) > 2:
            print("⛔ اسپم آخرین خبر")
            return

        # تمیزکاری هوشمند خط‌به‌خط
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات هوشمند با تمیزکاری خط‌به‌خط روشن شد.")
        with client:
