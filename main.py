...
# ===== امضاهایی که فقط حذف می‌شوند (نه بلاک) =====
TEXTS_TO_REMOVE = [
...
def is_spam_line(line):
    """تشخیص یک خط تبلیغاتی بر اساس الگوهای دعوت به عمل"""
...
def clean_text(text):
    ...
    # سپس امضاها و کلمات مشخص را حذف کن
...
async def handler(event):
    try:
        ...
        # اگر بعد از تمیزکاری چیزی باقی نماند، بلاک کن
        if is_rubbish(cleaned):
            print("⛔ بی‌ارزش (فقط تبلیغات بود)")
            return
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات هوشمند احسان (تشخیص ساختاری تبلیغات) روشن شد.")
        with client:
