...
def is_spam_line(line, prev_line=None, next_line=None):
    ...
    if re.search(r'(کارت\s?هدیه|غیر\s?حضوری|تحویل بگیر|با امید بانک|طرح ویژه|اعتبار|سپرده|سود علی‌الحساب)', line) and re.search(r'https?://', line):
        return True
    # خطوطی که فقط حاوی «وب‌سایت:» یا «تلگرام:» باشند (حتی بدون لینک)
    if re.fullmatch(r'\s*(وب‌سایت|تلگرام|اینستاگرام|واتس‌اپ)\s*:?\s*', line):
        return True
    # عبارات تبلیغاتی عمومی
    if any(phrase in line for phrase in PROMO_LINE_PHRASES):
        return True
    # اگر خط فعلی یکی از کلمات «وب‌سایت:» یا «تلگرام:» را داشته باشد و خط بعدی هم خالی یا شامل لینک باشد
    if next_line is not None:
        if re.search(r'(وب‌سایت|تلگرام|اینستاگرام)\s*:', line) and (not next_line.strip() or re.search(r'https?://', next_line)):
            return True
    # الگوی هوشمند: جمله‌ای که همزمان «اخبار» و «ببینید» / «دنبال کنید» داشته باشد،
    # اما فقط در صورتی حذف شود که خط قبلی یا بعدی شامل «وب‌سایت:» یا «تلگرام:» باشد.
...
def remove_source_attribution(text):
    """حذف خطوطی که صرفاً نام منبع هستند (ایرنا، ایسنا، یا ترکیباتشان)"""
    lines = text.split('\n')
    kept = []
    for line in lines:
        stripped = line.strip()
        # الگوهای مختلف ارجاع به منبع
...
def clean_text(text):
    ...
    # حذف خطوط منبع (ایرنا، ایسنا، ...)
    text = remove_source_attribution(text)
    # تبدیل هوشمندانه نام منابع به یاراخبر در صورتی که هنوز در متن باقی مانده باشند (مثل «به گزارش ایرنا»)
    # اما این کار را فقط روی خطوط کامل انجام ندهیم، چون ممکن است به اشتباه بخشی از متن خبر را تغییر دهد.
    # پس فقط خطوطی را که دقیقاً نام منبع هستند حذف کردیم.
    # پاکسازی نهایی
...
def format_news(cleaned_text):
    ...
    # حذف هشتگ‌های اضافی از تیتر (مثل #اینفو_ایرنا)
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات هوشمند (منبع‌زدایی کامل) روشن شد.")
        with client:
