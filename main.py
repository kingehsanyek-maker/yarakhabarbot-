...
BLOCKED_WORDS = [
    ...
    "منبع:", "پایگاه خبری", "تکمیلی", "بیشتر بخوانید:",
    "اینجا را کلیک کنید", "جهت کسب اطلاعات بیشتر",
...
recent_texts = []
recent_titles = []  # ذخیره هش تیترها برای تشخیص سریع‌تر
MAX_HISTORY = 300
...
def is_similar(new_text):
    # بررسی سریع تیتر (خط اول)
    lines = new_text.split('\n', 1)
    title = lines[0].strip()
    title_hash = hashlib.md5(title.encode()).hexdigest()
    if title_hash in recent_titles:
        return True
    # بررسی کامل متن با آستانه 0.8
...
if __name__ == "__main__":
    if not SESSION_STRING:
        ...
        print("🚀 ربات خبری (ایرنا + ایسنا + مشکات) نسخه ارتقا‌یافته روشن شد.")
        with client:
